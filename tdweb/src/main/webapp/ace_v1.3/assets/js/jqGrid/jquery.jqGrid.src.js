...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addCell = function(rowId,cell,pos,irow, srvr, rdata) {
			...
			// 过滤noticeBody中的html标签
			if(prp.indexOf("grid-table_noticeBody") > 0){
				return "<td role=\"gridcell\" "+prp+">"+$.jgrid.stripHtml(v)+"</td>";
			}else{
		...
		setPager = function (pgid, tp){
			...
			if(twd > 0) {
				if(pginp !== "") { 
					//twd += 50; //index页面导入js的情况下，去掉这行可以保证分页插件居中
				} //should be param
