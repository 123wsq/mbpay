...
(function (factory) {
	...
	var Tree = function (element, options) {
		...
		this.$element.on('click.fu.tree', '.tree-item', $.proxy( function(ev) {
			/**2015-02-28 xiejinzhong 选择节点时添加自定义处理函数调用*/
			var go = true;
			if(typeof options['selectItem'] === 'function'){
				go = options['selectItem'](ev);
			} 
			if(!(go === false)){
				this.selectItem(ev.currentTarget); 
			}
			/**2015-02-28 xiejinzhong 选择节点完成添加自定义处理函数调用*/
			if(typeof options['selectItemDone'] === 'function'){
				options['selectItemDone'](ev);
			}
		} ,this));
		...
		this.$element.on('click.fu.tree', '.tree-branch-header', $.proxy( function(ev) {
			/**2015-02-28 xiejinzhong 选择节点时添加自定义处理函数调用*/
			var go = true;
			if(typeof options['selectFolder'] === 'function'){
				go = options['selectFolder'](ev);
			} 
			if(!(go === false)){
				this.openFolder(ev.currentTarget); 
			}
			/**2015-02-28 xiejinzhong 选择节点完成添加自定义处理函数调用*/
			if(typeof options['selectFolderDone'] === 'function'){
				options['selectFolderDone'](ev);
			}
		}, this));
		//ACE
	...
	Tree.prototype = {
		...
		populate: function ($el) {
			...
			this.options.dataSource( treeData ? treeData : {} , function (items) {
				...
				if(typeof self.options['itemLoadDone'] === 'function'){
					self.options['itemLoadDone']($parent);
				}
			});
		},
